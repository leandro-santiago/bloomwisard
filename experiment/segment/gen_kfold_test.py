...
with open(base_path + data_name, "r") as fd:
    for line in fd:
        ...
        segment_label.append(int(split_data[-1].strip()) - 1)

        aux = []
        for s in split_data[:-1]:
            aux.append(float(s))

        segment_data.append(aux)
...
for i in range(n_fold):
    ...
    with open(base_path + str(i) + "-fold-test.data", "w") as fd:    
        for l in range(fold_size):
            for d in segment_data[j2][:-1]:
                fd.write(str(d) + ",")
            fd.write(str(segment_data[j2][-1]) + "\n")
            j2 += 1

    with open(base_path + str(i) + "-fold-train.label", "w") as fd:    
        for l in range(start):
            fd.write(str(segment_label[l]) + "\n")
    ...
    with open(base_path + str(i) + "-fold-train.data", "w") as fd:    
        for l in range(start):
            for d in segment_data[l][:-1]:
                fd.write(str(d) + ",")
            fd.write(str(segment_data[l][-1]) + "\n")
...
with open(base_path + "2b3-train.label", "w") as fd:    
    for i in range(t):
        for l in range(part_size):
            fd.write(str(segment_label[indexes[j1]]) + "\n")
            j1 += 1

with open(base_path + "2b3-train.data", "w") as fd:    
    for i in range(t):
        for l in range(part_size):
            for d in segment_data[indexes[j2]][:-1]:
                fd.write(str(d) + ",")
            fd.write(str(segment_data[indexes[j2]][-1]) + "\n")
            j2 += 1
